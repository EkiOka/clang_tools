...
import lib_40d60c18793c4117a0e52c0fdadfd4fc.adps.adp_copy       as copy
...
def not_tail(src:list)->list:
    ...
    return res
def deep_copy(src):
    return copy.deep_copy(src)

def deep_update(src,update,description:dict=dict()):
    """階層をたどってデータの上書きを行います。
    ...
    if isinstance(src,dict):
        if isinstance(update,dict):
            add_dict_keys = description.get("add_dict_keys",True)
            if add_dict_keys:
                res = dict()
                for key in src.keys():
                    res[key] = deep_copy(src[key])
                for key in update.keys():
                    if key in src.keys():
                        res[key] = deep_update(src[key],update[key],description)
                    else:
                        res[key] = deep_copy(update[key])
            else:
                res = deep_copy(update)
        else:
            match = False
    elif isinstance(src,list):
        if isinstance(update,list):
            add_list_items = description.get("add_list_items",True)
            if add_list_items:
                res:list = deep_copy(src)
                for item in update:
        ...
        if type(src) == type(update):
            res = update
        else:
            match = False

    if not match:
        raise_Exception(f"src({type(src)})とupdate{type(src)}で指定した型が違います。")
...
def caller_is_main()->bool:
    name = inspect.stack_local_var("__name__", None, 3)
    return bool(name == "__main__")
